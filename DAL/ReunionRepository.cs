...
namespace DAL
{
    public class ReunionRepository : BaseRepository, IRepository<Reunion>
    {
        ...
        public Response<Reunion> Insertar(Reunion entidad)
        {
            try
            {
                ...
                if (entidad.IdCandidato <= 0 || entidad.IdReclutador <= 0)
                    return new Response<Reunion>(
                        false,
                        "Debe especificar IdCandidato e IdReclutador válidos",
                        null,
                        null
                    );
                ...
                string sentencia = @"
                    INSERT INTO [Reunion] 
                        ([fecha], [enlaceMeet], [estadoConfirmacion], [idCandidato], [idReclutador]) 
                    VALUES 
                        (@fecha, @enlaceMeet, @estadoConfirmacion, @idCandidato, @idReclutador);
                ...
                using (SqlCommand comando = new SqlCommand(sentencia, conexion))
                {
                    ...
                    comando.Parameters.AddWithValue("@enlaceMeet", (object)entidad.EnlaceMeet ?? "");
        ...
        public Response<Reunion> Actualizar(Reunion entidad)
        {
            try
            {
                if (entidad == null || entidad.IdReunion <= 0)
                    return new Response<Reunion>(
                        false,
                        "Debe proporcionar una reunión válida",
                        null,
                        null
                    );
                ...
                using (SqlCommand comando = new SqlCommand(sentencia, conexion))
                {
                    ...
                    return new Response<Reunion>(
                        false,
                        "No se encontró la reunión especificada",
                        null,
                        null
                    );
                }
        ...
        // ==========================
        // OBTENER POR ID (con datos de candidato y reclutador)
        // ==========================
        public Response<Reunion> ObtenerPorId(int id)
        {
            try
            {
                ...
                string sentencia = @"
                    SELECT 
                        r.[idReunion], r.[fecha], r.[enlaceMeet], r.[estadoConfirmacion], 
                    ...
                    INNER JOIN [Candidato] c 
                        ON r.[idCandidato] = c.[idCandidato]
                    INNER JOIN [Usuario] uc 
                        ON c.[idCandidato] = uc.[idUsuario]
                    INNER JOIN [Reclutador] rec 
                        ON r.[idReclutador] = rec.[idReclutador]
                    INNER JOIN [Usuario] ur 
                        ON rec.[idReclutador] = ur.[idUsuario]
                ...
                using (SqlCommand comando = new SqlCommand(sentencia, conexion))
                {
                    ...
                    using (SqlDataReader reader = comando.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            Reunion reunion = MapearReunionExtendida(reader);
        ...
        // ==========================
        // OBTENER TODOS (para panel Programar Entrevistas)
        // ==========================
        public Response<Reunion> ObtenerTodos()
        {
            try
            {
                ...
                using (SqlCommand comando = new SqlCommand(sentencia, conexion))
                {
                    ...
                    using (SqlDataReader reader = comando.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            lista.Add(MapearReunionExtendida(reader));
                        }
                    }
                }
                ...
                if (lista.Count > 0)
                    return new Response<Reunion>(
                        true,
                        $"Se encontraron {lista.Count} reuniones",
                        null,
                        lista
                    );
            ...
            catch (Exception ex)
            {
                return new Response<Reunion>(
                    false,
                    $"Error al obtener todas las reuniones: {ex.Message}",
                    null,
                    null
                );
            }
        ...
        // MAPEADOR EXTENDIDO
        // ==========================
        private Reunion MapearReunionExtendida(SqlDataReader reader)
        {
            return new Reunion
            {
                ...
                // Campos "extendidos" para panel, mails, n8n:
